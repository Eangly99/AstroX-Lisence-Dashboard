...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-card border-r border-border text-zinc-300">
      {/* Header */}
      <div className="flex items-center justify-between h-16 px-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white tracking-wider text-sm xl:block md:hidden">
            ASTROX LICENSE
          </span>
        </div>
        <button
        ...
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium relative transition-colors duration-150 ${
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-primary/10 border-l-2 border-primary rounded-r"
                  transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                />
              )}
              <Icon className="w-5 h-5 flex-shrink-0 z-10" />
              <span className="xl:block md:hidden z-10 transition-opacity duration-150">
                {item.label}
              </span>
            </Link>
          );
      ...
      {/* Footer */}
      <div className="p-4 border-t border-border text-xs text-zinc-500 xl:block md:hidden">
